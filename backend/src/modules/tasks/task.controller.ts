import type { FastifyReply, FastifyRequest } from "fastify";

import type { CreateTaskRequest, UpdateTaskRequest } from "./task.service.js";
import { taskService } from "./task.service.js";
import type { TaskQueryParams } from "./task.types.js";
import { AppError } from "../../utils/app-error.js";
import { successDataResponse } from "../../utils/api-response.js";

function getAuthUser(request: FastifyRequest) {
  if (!request.authUser) throw new AppError("Unauthorized", 401);
  return request.authUser;
}

function handleError(error: unknown, reply: FastifyReply): void {
  if (error instanceof AppError) {
    reply.status(error.statusCode).send({ success: false, message: error.message });
    return;
  }
  throw error;
}

export async function listTasks(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  try {
    const result = await taskService.listTasks(
      getAuthUser(request),
      request.query as TaskQueryParams,
    );
    reply.status(200).send(successDataResponse(result));
  } catch (error) {
    handleError(error, reply);
  }
}

export async function getTask(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  try {
    const { id } = request.params as { id: string };
    const task = await taskService.getTask(getAuthUser(request), id);
    reply.status(200).send(successDataResponse({ task }));
  } catch (error) {
    handleError(error, reply);
  }
}

export async function createTask(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  try {
    const task = await taskService.createTask(
      getAuthUser(request),
      request.body as CreateTaskRequest,
    );
    reply.status(201).send(successDataResponse({ task }));
  } catch (error) {
    handleError(error, reply);
  }
}

export async function updateTask(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  try {
    const { id } = request.params as { id: string };
    const task = await taskService.updateTask(
      getAuthUser(request),
      id,
      request.body as UpdateTaskRequest,
    );
    reply.status(200).send(successDataResponse({ task }));
  } catch (error) {
    handleError(error, reply);
  }
}
